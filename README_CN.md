...
&emsp;&emsp;通过这组宏，程序员可以自由地以任何风格编写应用程序，包括**事件触发的调度**、**优先级驱动的调度**，或**仅使用表驱动的调度**。所有任务执行都是从运行到完成，因此单个线程内不可能发生抢占。但是，可以将**中断处理程序**编写为任务（甚至是托管多个任务的线程）来规避此限制，并且通过硬件中断嵌套实现多个抢占优先级。
